...
Также мы скачали дамп базы mysql, который содержит личные данные посетителей сайта.
...
В папке Share найдены файлы с расширением .cert. 

Найден файл Ransom.ps1. Внутри оказался код шифрования AES (CBC).
...
Сервер был обнаружен, были попытки побрутить пароли, зная логин `admin`, но безуспешные, так как система работала медленно, и часто "подлагивало", что делало работу в Burp Suite при включенном Intruder невозможной.
